...
1. 读取 `client.js`（约 140KB，包含 3 张壁纸与「无处可逃！」大招语音的 base64 内嵌素材）。
2. 调用 `cordis_define`：
...
4. 完成后告知用户：页面右下角出现刻晴挂件（最大 320×430px 自适应画框，整张立绘完整展示）；左键点击释放元素爆发「天街巡游」（语音「无处可逃！」+ 雷光特效）；🖼 切换壁纸；右键菜单（释放大招 / 切换壁纸 / 收起 / 一键卸载）。
...
- 大招技能名：元素爆发「天街巡游」
- 大招语音（其一）：无处可逃！（BWIKI 原文件：`https://patchwiki.biligame.com/images/ys/b/bd/rogvhc1yk82jdpolwede3ukp28lb7th.mp3`）
